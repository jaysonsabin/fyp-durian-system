...
export default function FarmCreationLock({ onAddFarm, onLogout }) {
  ...
  return (
    <div className="h-screen w-screen flex flex-col justify-center items-center bg-gray-900 p-6 relative overflow-hidden">
      ...
      <div className="bg-white p-8 rounded-lg w-full max-w-md shadow-2xl relative z-10 text-center border border-gray-100/50">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4 text-left">
          <div>
            <label className="block text-xs font-bold text-gray-500 uppercase ml-2 mb-1.5">{t('plantation_name')}</label>
            <input 
              type="text" 
              placeholder={t('placeholder_farm_name')}
              required
              className="w-full p-4 bg-gray-50 border border-gray-200 rounded-lg outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent text-gray-800 font-medium transition-all"
              value={farmName}
          ...
          <div>
            <label className="block text-xs font-bold text-gray-500 uppercase ml-2 mb-1.5">{t('location_region')}</label>
            <input 
              type="text" 
          ...
          <button 
            ...
            className={`w-full text-white py-4.5 mt-4 rounded-lg font-bold shadow-lg shadow-green-600/10 transition-all duration-300 ${
              isSubmitting 
